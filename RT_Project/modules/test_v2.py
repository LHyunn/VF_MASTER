...
class TestModel:
    ...
    def test_model(self, verbose=True, upload=True):
        for model in self.model_list:
            ...
            if os.path.exists(result_path + f"/threshold_recall_1.csv"):
                print("Already tested.")
                self.threshold, f1_score_, recall_, precision_, accuracy_= self.draw_threshold_change_curves(model_name, result_path)
                ...
                with mirrored_strategy.scope():
                    ...
                    tf_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=float(Learning_Rate)), loss=Loss_Function, metrics=["accuracy", f1_m, precision_m, recall_m])
                self.test_pred = tf_model.predict(self.test_image_list) # 1. predict
                df = pd.DataFrame(self.test_pred)
                df.to_csv(result_path + f"/pred.csv", index=False)
    ...
    def draw_threshold_change_curves(self, model_name, result_path):
        if os.path.exists(result_path + f"/threshold_recall_1.csv"):
            df = pd.read_csv(result_path + f"/threshold_recall_1.csv")
            df = df[df["F1_score"] == df["F1_score"].max()]
            max_F1_score_Threshold, f1_score_, recall_, precision_, accuracy_ = df.iloc[0]
            ...
            df = pd.DataFrame({"Threshold": Threshold_list, "F1_score": F1_score_list, "Recall": Recall_list, "Precision": Precision_list, "Accuracy": Accuracy_list})
